...
def check_regulatory_news():
    """
    Check for RBI/SEBI announcements.
    """
    try:
        ...
        for article in articles:
            ...
            if any(word in content for word in ['rbi', 'sebi']) and any(word in content for word in ['announce', 'decision', 'policy', 'change', 'rule']):
...
_This may impact market sentiment._"""
        ...
        logger.error(f"Regulatory news error: {e}")
...
def run_scheduler():
    ...
    while True:
        try:
            ...
            # Regulatory News check every 2 hours
            ...
            if updates and updates.get('ok'):
                for update in updates.get('result', []):
                    try:
                        ...
                        # Handle commands without required arguments
                        elif cmd == '/price':
                            send_message(chat_id, "Usage: /price SYMBOL\nExample: /price RELIANCE")
                        elif cmd == '/stock':
                            send_message(chat_id, "Usage: /stock SYMBOL\nExample: /stock TCS")
                        elif cmd == '/chart':
                            send_message(chat_id, "Usage: /chart SYMBOL\nExample: /chart INFY")
                        elif cmd == '/alert':
                            send_message(chat_id, "Usage: /alert SYMBOL PRICE\nExample: /alert RELIANCE 2500")
                        elif cmd == '/days':
                            send_message(chat_id, "Usage: /days SYMBOL [N]\nExample: /days RELIANCE 15")
                        else:
